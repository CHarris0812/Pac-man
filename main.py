...
def trainAI(generation = 0, path = "C:\\Users\\CWHar\\Downloads\\Coding\\Personal\\PacMan\\AI_Models", modelsPerGeneration = 1, maxGenerations = 3):
    if generation == 0:
        folderPath = os.path.join(path, "gen_0")
        if "gen_0" not in os.listdir(path):
            os.mkdir(folderPath)
        for i in range(modelsPerGeneration):
            score, ai = runNoGraphics(AIMode = True, speedUpFactor = 100)
            print("Run " + str(i) + ", score = " + str(score))
            torch.save(ai, os.path.join(folderPath, str(i) + "-" + str(score)))

    for i in range(generation, maxGenerations + 1):
        trainAIGeneration(i, path, modelsPerGeneration)


def trainAIGeneration(generation, path, modelsPerGeneration):
    previousGenPath = os.path.join(path, "gen_" + str(generation - 1))
    folderPath = os.path.join(path, "gen_" + str(generation))
    os.mkdir(folderPath)

    files = os.listdir(previousGenPath)
    bestModels = findTopModels(files, max(1, modelsPerGeneration // 10))

    for i in range(len(bestModels)):
        for j in range(10):
            model = torch.load(os.path.join(previousGenPath, bestModels[i]))
            for param in model.parameters():
                with torch.no_grad():
                    param += torch.randn(param.size()) * .02
            
            score, ai = runNoGraphics(AIMode = True, speedUpFactor = 100)
            print("Run " + str(i * 10 + j + generation * modelsPerGeneration) + ", score = " + str(score))
            torch.save(ai, os.path.join(folderPath, str(i * 10 + j) + "-" + str(score)))
    return

def runModel(path):
    modelToRun = torch.load(path)
    for param in modelToRun.model.parameters():
        print(param)
    run(True, modelToRun)

#trainAI(generation = 0)
runModel(r"C:\Users\CWHar\Downloads\31-346")